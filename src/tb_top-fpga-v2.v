...
// Target Design: top_fpga / top_module-v5
// Description: Vivado Testbench for MAX30102 Pulse Oximeter FPGA System.
...
module tb_top_fpga;
    ...
    // Fixed port list matching top_fpga.v (clk, rst_n, i2c_sda, i2c_scl, uart_tx, led)
    // ------------------------------------------------------------------------
    top_fpga dut (
        .clk     (clk_100m),
        .rst_n   (rst_n),
    ...
    // Monitor SCL falling edges to respond with ACK
    ...
    // ------------------------------------------------------------------------
    always @(led) begin
        $display("[LED UPDATE @ %0t ns] Sample_Blink=%b | UART_Busy=%b | I2C_Error=%b | SpO2_LEDs=8'd%0d",
                 $time, led[0], led[1], led[2], led[15:8]);
    end
